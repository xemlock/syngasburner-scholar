...
<?php   if ($sections) { ?>
[section="<?php $this->displayAttr($year) ?>" collapsible="<?php echo intval($year < year('Y')) ?>" collapsed="1"]
<?php   } ?>
...
<?php     } ?>
    [__tag="cite" class="title"][url="<?php $this->displayAttr($conference->url) ?>"]<?php $this->display($conference->title) ?>[/url][/__tag]<?php 
          ...
          if ($conference->locality) { ?>, [__tag="span" class="adr"][__tag="span" class="locality"]<?php $this->display($conference->locality) ?>[/__tag]<?php          if ($conference->country) { ?>, [__tag="span" class="country-name" title="<?php $this->displayAttr($conference->country) ?>"]<?php $this->display($conference->country_name) ?>[/__tag]<?php } ?>[/__tag]
<?php     }  ?>
  [/__tag]
